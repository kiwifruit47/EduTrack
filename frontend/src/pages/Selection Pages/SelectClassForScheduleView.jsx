...
function SelectClassForScheduleView() {
  const { t } = useTranslation();
  const navigate = useNavigate();

  const [grouped, setGrouped]   = useState({});   // { schoolName: [classDto, ...] }
  ...
  return (
    <Layout>
      <Box sx={{ p: 3 }}>
        <Typography variant="h5" sx={{ mb: 3 }}>{t('selectClass.title')}</Typography>
            ...
            <Box key={schoolName} sx={{ mb: 4 }}>
              ...
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2 }}>
                {classes.map(cls => (
                  ...
                  >
                    <CardActionArea onClick={() => navigate(`/schedule/${cls.id}`)}>
...
export default SelectClassForScheduleView;
