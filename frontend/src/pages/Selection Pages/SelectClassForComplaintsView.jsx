...
function SelectClassForComplaintsView() {
  ...
  return (
    <Layout>
      <Box sx={{ p: 3 }}>
        <Typography variant="h5" sx={{ mb: 3 }}>{t('selectClass.titleComplaints')}</Typography>
              ...
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2 }}>
                {classes.map(cls => (
                  <Card key={cls.id} sx={{ width: 140 }}>
                    <CardActionArea onClick={() => navigate(`/complaints/class/${cls.id}`)}>
...
export default SelectClassForComplaintsView;
